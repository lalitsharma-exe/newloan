...
use App\Models\{AuditLog, Loan, LoanInstallment, LoanProduct, Payment};
...
class LoanController extends Controller {
    ...
    public function recordPayment(Request $request, Loan $loan) {
        ...
        AuditLog::record('loan.record_payment', "Manual payment L{$request->amount} on {$loan->loan_number}", $loan, [], ['amount' => $request->amount, 'method' => $request->method]);
        return redirect()->route('admin.loans.show', $loan)->with('success', "Payment of L{$request->amount} recorded.");
    }
    ...
    public function reversePayment(Request $request, Loan $loan) {
        ...
        AuditLog::record('loan.reverse_payment', "Reversed payment L{$oldAmount} on {$loan->loan_number}: {$request->reason}", $loan);
    ...
    public function waiveInstallment(Request $request, Loan $loan) {
        ...
        AuditLog::record('loan.waive_installment', "Waived installment #{$inst->installment_number} (L{$waived}) on {$loan->loan_number}: {$request->reason}", $loan);
    ...
    public function addLateFee(Request $request, Loan $loan) {
        ...
        AuditLog::record('loan.add_late_fee', "Added late fee L{$request->fee_amount} to installment #{$inst->installment_number} on {$loan->loan_number}", $loan);
        return redirect()->route('admin.loans.show', $loan)->with('success', "Late fee of L{$request->fee_amount} added.");
    }
    ...
    public function disburse(Request $request, Loan $loan) {
        $request->validate(['disbursement_date' => 'required|date']);
        $loan->update(['status' => 'active', 'disbursement_date' => $request->disbursement_date]);
        AuditLog::record('loan.disburse', "Loan {$loan->loan_number} disbursed", $loan);
        return redirect()->route('admin.loans.show', $loan)->with('success', 'Loan marked as disbursed.');
    }
    ...
    public function restructure(Request $request, Loan $loan) {
        $request->validate([
            'new_term'          => 'required|integer|min:1|max:120',
    ...
    public function agreement(Loan $loan) {
        ...
        // If DomPDF installed: return \Barryvdh\DomPDF\Facade\Pdf::loadView('admin.loans.agreement-pdf', compact('loan'))->download('Agreement-'.$loan->loan_number.'.pdf');
    ...
    // ── Settlement Letter (paid-in-full) ────────────────────────────
    ...
    public function export(Request $request) {
        ...
        foreach ($loans as $l) {
            $csv .= implode(',', [
                ...
                $l->status,
                $l->disbursement_date?->format('Y-m-d'),
            ]) . "\n";
        }
        return response($csv, 200, [
            'Content-Type'        => 'text/csv',
            'Content-Disposition' => 'attachment; filename="loans-'.now()->format('Y-m-d').'.csv"',
        ]);
    }

    // ── Overdue list ────────────────────────────────────────────────
    public function overdue(Request $request) {
        return $this->index($request->merge(['status' => 'overdue']));
    }

    // ── AJAX loan lookup (for bulk repayment autocomplete) ──────────
    public function lookup(Request $request) {
        $ln   = $request->input('loan_number', '');
        $loan = Loan::where('loan_number', 'like', "%{$ln}%")
        ...
        return response()->json([
            ...
            'next_due'      => $nextInst?->outstanding_amount ?? $loan->monthly_installment,
            'due_date'      => $nextInst?->due_date?->format('d M Y'),
            'status'        => $loan->status,
        ]);
    }

    // ── Bulk repayment entry (up to 30 at once) ─────────────────────
    public function bulkRepayment(Request $request) {
        if ($request->isMethod('GET')) {
            return view('admin.loans.bulk-repayment');
        }
        $request->validate([
            'method'           => 'required|in:cash,bank_transfer,mobile_money,card,cheque,payroll',
            'rows'             => 'required|array|min:1|max:30',
            'rows.*.loan_number' => 'required|string',
        ...
        AuditLog::record('loan.bulk_repayment',
            "Bulk repayment: {$results['success']} recorded, {$results['failed']} failed",
            null, [], ['method' => $request->method]
        );

        return redirect()->route('admin.loans.bulk-repayment')
            ->with('bulk_results', $results)
            ->with('success', "{$results['success']} payment(s) recorded successfully." .
                ($results['failed'] ? " {$results['failed']} failed." : ''));
    }

    // ── Bulk loan import via CSV ─────────────────────────────────────
