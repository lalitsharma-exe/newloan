...
class DashboardService {
    public function getStats():array {
        return [
            ...
            "applications_pending"    => LoanApplication::pending()->count(),
            "overdue_loans"           => Loan::overdue()->count(),
            "expected_collections"    => LoanInstallment::whereDate("due_date",">=",now())->whereDate("due_date","<=",now()->endOfMonth())->whereIn("status",["pending","partial","overdue"])->sum("outstanding_amount"),
            "total_portfolio"         => Loan::active()->sum("outstanding_balance"),
            "total_disbursed_month"   => Loan::whereMonth("disbursement_date",now()->month)->whereYear("disbursement_date",now()->year)->sum("principal_amount"),
            "total_borrowers"         => User::borrowers()->count(),
        ];
    }
    public function getRecentApplications(int $limit=10) {
        return LoanApplication::with(["user","loanProduct"])->where("status","!=","draft")->latest()->limit($limit)->get();
    }
    public function getOverdueLoans(int $limit=5) {
        return Loan::with("user")->overdue()->orderByDesc("outstanding_balance")->limit($limit)->get();
    }
