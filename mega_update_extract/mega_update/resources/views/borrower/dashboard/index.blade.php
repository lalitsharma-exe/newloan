...
<div style="display:grid;grid-template-columns:repeat(3,1fr);gap:18px;margin-bottom:24px">
  ...
  {{-- CARD 1: Amount Due (blue) --}}
  <div style="background:#fff;border-radius:16px;border:2px solid #3b82f6;padding:24px;display:flex;flex-direction:column;align-items:flex-start;gap:0">
    <div style="display:flex;align-items:center;gap:8px;margin-bottom:14px">
      <div style="width:36px;height:36px;background:rgba(59,130,246,.1);border-radius:9px;display:flex;align-items:center;justify-content:center">
        <i class="bi bi-exclamation-triangle-fill" style="color:#3b82f6;font-size:16px"></i>
      </div>
      <div style="font-size:16px;font-weight:700;color:#1e3a8a">Amount Due</div>
    </div>
    <div style="width:100%;border-top:1px solid #eff6ff;margin-bottom:16px"></div>
    @if($nextInst)
    <div style="font-family:'Cormorant Garamond',serif;font-size:42px;font-weight:800;color:#3b82f6;line-height:1;margin-bottom:6px">
      M{{ number_format($nextInst->outstanding_amount,0) }}
    </div>
    <div style="font-size:13px;color:#64748b;margin-bottom:20px">
      Due Date: {{ $nextInst->due_date->format('d M') }}
    </div>
    <a href="{{ route('borrower.payments.make') }}"
       style="display:block;width:100%;text-align:center;background:#3b82f6;color:#fff;padding:11px;border-radius:10px;font-size:14px;font-weight:800;letter-spacing:.06em;text-decoration:none;transition:background .2s">
      PAY NOW
    </a>
    @else
  ...
  {{-- CARD 2: Overdue Amount (amber) --}}
  <div style="background:#fff;border-radius:16px;border:2px solid #f59e0b;padding:24px;display:flex;flex-direction:column;align-items:flex-start">
    <div style="display:flex;align-items:center;gap:8px;margin-bottom:14px">
    ...
    <div style="width:100%;border-top:1px solid #fffbeb;margin-bottom:16px"></div>
    @if($overdueInst)
  ...
  <div style="background:#fff;border-radius:16px;border:2px solid #16a34a;padding:24px;display:flex;flex-direction:column;align-items:flex-start">
    <div style="display:flex;align-items:center;gap:8px;margin-bottom:14px">
    ...
    <div style="display:flex;flex-direction:column;gap:8px;width:100%;margin-bottom:20px">
      ...
      <div style="display:flex;justify-content:space-between;font-size:14px">
        ...
        <span style="font-weight:700;color:#dc2626">M{{ number_format($activeLoan->outstanding_balance,0) }}</span>
      </div>
      @if($recentPayment)
      <div style="display:flex;justify-content:space-between;font-size:14px">
        <span style="color:#64748b">Recent Payment</span>
        <span style="font-weight:700;color:#16a34a">M{{ number_format($recentPayment->amount,0) }}</span>
      </div>
      @endif
      {{-- Progress --}}
      <div style="margin-top:6px">
        ...
        <div style="background:#dcfce7;border-radius:99px;height:7px">
          <div style="background:#16a34a;height:100%;width:{{ $pct }}%;border-radius:99px;transition:width .5s"></div>
        </div>
      </div>
    </div>
    <a href="{{ route('borrower.loans.show', $activeLoan) }}"
       style="display:block;width:100%;text-align:center;background:#16a34a;color:#fff;padding:11px;border-radius:10px;font-size:14px;font-weight:700;text-decoration:none">
      View Details
    </a>
  </div>
