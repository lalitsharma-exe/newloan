...
<head>
<meta charset="utf-8">
<title>Consolidated Settlement Quotation — {{ $user->name }}</title>
...
.breakdown-table td {padding:14px 20px;font-size:13px;border-bottom:1px solid var(--border)}
    ...
    <div class="body">
        ...
        <table class="breakdown-table">
            ...
            <tbody>
                ...
                @php 
                    $loanOutstanding = $loan->installments()->whereNotIn('status', ['paid', 'waived'])->sum('outstanding_amount');
                @endphp
                <tr>
                    ...
                    <td style="text-align:right; font-weight:700">M {{ number_format($loanOutstanding, 2) }}</td>
                </tr>
                @endforeach
                <tr class="total-row">
                    <td colspan="3">GRAND TOTAL CONSOLIDATED SETTLEMENT</td>
                    <td style="text-align:right">M {{ number_format($totalOutstanding, 2) }}</td>
                </tr>
            </tbody>
        </table>
        ...
        <div class="payment-box">
            <div class="p-instr">
                ...
                <div style="font-size:13px;line-height:1.6;color:#1e40af">
                    Please pay the total amount of <strong>M {{ number_format($totalOutstanding, 2) }}</strong> by <strong>{{ $validUntil }}</strong>.
                    <br><br>
