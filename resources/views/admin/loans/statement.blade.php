...
@section('bc','<a href="'.route('admin.loans.index').'">Loans</a> / <a href="'.route('admin.loans.show',$loan).'">{{ $loan->loan_number }}</a> / Statement')
@section('content')
...
<div style="max-width:800px">
  ...
  <div style="display:flex;gap:10px;margin-bottom:20px;justify-content:flex-end">
    ...
    <a href="{{ route('admin.loans.export', ['ids'=>[$loan->id]]) }}" class="btn btn-p"><i class="bi bi-download"></i> Export CSV</a>
  </div>
    ...
    <div style="padding:28px 36px">
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:24px;margin-bottom:28px">
        ...
        <div>
          ...
          @foreach(['Principal'=>'L '.number_format($loan->principal_amount,2),'Interest Rate'=>$loan->interest_rate.'%/mo','Term'=>$loan->term_months.' months','Monthly Payment'=>'L '.number_format($loan->monthly_installment,2),'Disbursed'=>$loan->disbursement_date?->format('d M Y')??'—','Outstanding'=>'L '.number_format($loan->outstanding_balance,2)] as $l=>$v)
      ...
      <table style="width:100%;border-collapse:collapse;margin-bottom:28px">
        ...
        <tbody>
          ...
          <tr>
            ...
            <td style="padding:10px 12px;font-size:13px;font-weight:700;border-bottom:1px solid var(--border);text-align:right">L{{ number_format($p->amount,2) }}</td>
        ...
        <tfoot>
          <tr style="background:#f8fafc">
            ...
            <td style="padding:12px;font-weight:800;font-size:15px;color:#10b981;text-align:right">L{{ number_format($loan->payments->where('status','verified')->sum('amount'),2) }}</td>
            <td></td>
          </tr>
        </tfoot>
      </table>
