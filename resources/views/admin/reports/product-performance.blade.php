...
<div class="card mb-4">
  <div class="card-header">
    <span class="card-title">Product Performance Report</span>
    <form method="POST" action="{{ route('admin.reports.export') }}" style="display:inline">@csrf<input type="hidden" name="report_type" value="{{ str_replace('-','_','product-performance') }}"><input type="hidden" name="format" value="csv"><button class="btn btn-sm btn-outline"><i class="bi bi-download"></i> Export CSV</button></form>
  </div>
  <div class="card-body">
