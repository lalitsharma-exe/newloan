...
@section('page-title','Repayment Report')
@section('content')
<div class="card mb-4">
  <div class="card-header">
    <span class="card-title">Repayment Report</span>
    <form method="POST" action="{{ route('admin.reports.export') }}" style="display:inline">@csrf<input type="hidden" name="report_type" value="{{ str_replace('-','_','repayment') }}"><input type="hidden" name="format" value="csv"><button class="btn btn-sm btn-outline"><i class="bi bi-download"></i> Export CSV</button></form>
  </div>
  <div class="card-body">
...
<div class="card">
  <div class="card-body">
    ...
    <tr><td style="font-weight:700;color:#4f46e5">{{ $loan->loan_number }}</td><td>{{ $loan->user->name??'—' }}</td><td>{{ $loan->loanProduct->name??'—' }}</td><td>M {{ number_format($loan->principal_amount,0) }}</td><td>M {{ number_format($loan->outstanding_balance,0) }}</td><td><span class="badge badge-{{ $loan->status==='active'?'success':($loan->status==='overdue'?'danger':'secondary') }}">{{ ucfirst($loan->status) }}</span></td></tr>
    ...
    <tr><td style="font-weight:700;color:#4f46e5">{{ $p->payment_reference }}</td><td>{{ $p->loan->user->name??'—' }}</td><td>M {{ number_format($p->amount,2) }}</td><td>{{ ucfirst(str_replace('_',' ',$p->method)) }}</td><td>{{ $p->created_at->format('d M Y') }}</td></tr>
