...
@section('content')
    ...
    <div class="d-wrap">
        ...
        @if($stats['exec_par30'] > 0 || $stats['liq_runaway'] == 0 || $stats['loans_due_today'] > 0)
            <div class="alert-bar"
                style="background: #fffbeb; border: 1px solid #fde68a; border-radius: 12px; padding: 12px 20px; margin-bottom: 24px; display: flex; align-items: center; gap: 15px;">
                <div
                    style="background: #fef3c7; color: #d97706; width: 32px; height: 32px; border-radius: 50%; display: flex; align-items: center; justify-content: center;">
                ...
                <div style="flex: 1;">
                    ...
                    <div style="font-size: 12px; color: #b45309;">
                        @if($stats['exec_par30'] > 0) <span style="margin-right: 15px;">• PAR 30 is currently
                        {{ $stats['exec_par30'] }}%</span> @endif
                        @if($stats['liq_runaway'] == 0) <span style="margin-right: 15px;">• Liquidity runway is critical (0
                        days)</span> @endif
                        @if($stats['loans_due_today'] > 0) <span>• {{ $stats['loans_due_today'] }} installments due today</span>
                        @endif
                    </div>
                </div>
        ...
        <div class="kpi-grid kpi-row-5">
            <div class="kpi-card" style="border-top: 3px solid #3b82f6;">
                <div class="kpi-head">
                    <div class="kpi-icon" style="background: #eff6ff; color: #3b82f6;"><i class="bi bi-currency-dollar"></i>
                    </div>
                </div>
                <div class="kpi-label">TOTAL REVENUE ({{ $periodLabel }})</div>
                @if(isset($chg['revenue']))
                    <div class="kpi-trend {{ $chg['revenue'] >= 0 ? 'trend-up' : 'trend-down' }}">
                {{ $chg['revenue'] >= 0 ? '↑' : '↓' }} {{ abs($chg['revenue']) }}%</div> @endif
            ...
            <div class="kpi-card" style="border-top: 3px solid #8b5cf6;">
                <div class="kpi-head">
                    <div class="kpi-icon" style="background: #f5f3ff; color: #8b5cf6;"><i class="bi bi-wallet2"></i></div>
                </div>
                <div class="kpi-label">NET PROFIT ({{ $periodLabel }})</div>
                @if(isset($chg['profit']))
                    <div class="kpi-trend {{ $chg['profit'] >= 0 ? 'trend-up' : 'trend-down' }}">
                {{ $chg['profit'] >= 0 ? '↑' : '↓' }} {{ abs($chg['profit']) }}%</div> @endif
            ...
            <div class="kpi-card" style="border-top: 3px solid #10b981;">
                <div class="kpi-head">
                    <div class="kpi-icon" style="background: #f0fdf4; color: #10b981;"><i class="bi bi-bank"></i></div>
                </div>
            ...
            <div class="kpi-card" style="border-top: 3px solid {{ $stats['exec_par30'] > 0 ? '#f59e0b' : '#84cc16' }};">
                <div class="kpi-head">
                    <div class="kpi-icon"
                        style="background: {{ $stats['exec_par30'] > 0 ? '#fffbeb' : '#f7fee7' }}; color: {{ $stats['exec_par30'] > 0 ? '#f59e0b' : '#84cc16' }};">
                        <i class="bi bi-arrow-repeat"></i></div>
                </div>
                <div class="kpi-label">PAR 30</div>
                <div
                    class="kpi-trend {{ $stats['exec_par30'] > 5 ? 'trend-down' : ($stats['exec_par30'] > 0 ? 'trend-neutral' : 'trend-up') }}">
                    {{ $stats['exec_par30'] > 5 ? '↑ Critical' : ($stats['exec_par30'] > 0 ? '↑ Warning' : '↑ Healthy') }}
                </div>
                <div class="kpi-val" style="color: {{ $stats['exec_par30'] > 0 ? '#f59e0b' : '#1e293b' }}">
                    {{ $stats['exec_par30'] }}%</div>
                <div class="kpi-sub-val">Portfolio at risk</div>
            </div>
            <div class="kpi-card" style="border-top: 3px solid #f59e0b;">
                <div class="kpi-head">
                    <div class="kpi-icon" style="background: #fffbeb; color: #f59e0b;"><i class="bi bi-bullseye"></i></div>
                </div>
                <div class="kpi-label">COLLECTION RATE</div>
                <div class="kpi-trend {{ $stats['exec_collection_rate'] < 90 ? 'trend-down' : 'trend-up' }}">
                    {{ $stats['exec_collection_rate'] < 90 ? '↓ Low' : '↑ High' }}</div>
        ...
        <div class="kpi-grid kpi-row-6">
            <div class="kpi-card"
                style="border-top: 3px solid {{ $stats['liq_cash_available'] <= $stats['liq_undisbursed'] ? '#dc2626' : '#16a34a' }};">
                <div class="kpi-label">CASH AVAILABLE</div>
                <div class="kpi-val"
                    style="color: {{ $stats['liq_cash_available'] <= $stats['liq_undisbursed'] ? '#dc2626' : '#16a34a' }};">
                    M{{ number_format($stats['liq_cash_available'], 2) }}
                </div>
                <div class="kpi-sub-val">
                    {{ $stats['liq_cash_available'] <= 0 ? 'No funds available' : 'Bank balance (est)' }}</div>
                <div class="kpi-footer">
                    <span>Utilization</span>
                    <div class="kpi-progress">
                        <div class="progress-bar"
                            style="width: 25%; background: {{ $stats['liq_cash_available'] <= $stats['liq_undisbursed'] ? '#dc2626' : '#16a34a' }};">
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="kpi-card" style="border-top: 3px solid #0284c7;">
                ...
                <div class="kpi-footer">
                    ...
                    <div class="kpi-progress">
                        <div class="progress-bar" style="width: 73%; background: #0284c7;"></div>
                    </div>
            ...
            <div class="kpi-card" style="border-top: 3px solid #92400e;">
                ...
                <div class="kpi-footer">
                    <span>vs Inflow</span>
                    <div class="kpi-progress">
                        <div class="progress-bar" style="width: 37%; background: #ea580c;"></div>
                    </div>
            ...
            <div class="kpi-card" style="border-top: 3px solid #dc2626;">
                ...
                @if($stats['liq_net_liquidity'] < 0)
                <div class="kpi-trend trend-down" style="background:#fef2f2; color:#dc2626">⚠ Alert</div> @else <div
                class="kpi-trend trend-up">Stable</div> @endif
                <div class="kpi-val" style="color: {{ $stats['liq_net_liquidity'] < 0 ? '#dc2626' : '#1e293b' }}">
                    M{{ number_format($stats['liq_net_liquidity'], 2) }}</div>
            ...
            <div class="kpi-card" style="border-top: 3px solid {{ $stats['liq_runaway'] == 0 ? '#dc2626' : '#64748b' }};">
                ...
                <div class="kpi-tag"
                    style="background: {{ $stats['liq_runaway'] == 0 ? '#fef2f2' : '#f1f5f9' }}; color: {{ $stats['liq_runaway'] == 0 ? '#dc2626' : '#64748b' }};">
        ...
        <div class="kpi-grid kpi-row-6">
            ...
            <div class="kpi-card" style="border-top: 3px solid #6366f1;">
                <div class="kpi-head">
                    <div class="kpi-icon" style="background:#eef2ff; color:#6366f1"><i class="bi bi-arrow-repeat"></i></div>
                </div>
            ...
            <div class="kpi-card" style="border-top: 3px solid #db2777;">
                <div class="kpi-head">
                    <div class="kpi-icon" style="background:#fdf2f8; color:#db2777"><i class="bi bi-heart"></i></div>
                </div>
            ...
            <div class="kpi-card" style="border-top: 3px solid #475569;">
                <div class="kpi-head">
                    <div class="kpi-icon" style="background:#f8fafc; color:#475569"><i class="bi bi-stack"></i></div>
                </div>
        ...
        <div class="body-grid" style="margin-top: 24px;">
            <div class="body-left">
                            ...
                            <tbody>
                                @foreach($recentApplications->take(6) as $app)
                                    <tr>
                                        ...
                                        <td><a href="{{ route('admin.applications.show', $app->id) }}" class="ico-btn"><i
                ...
                <div class="card" style="margin-top:20px">
                    <div class="card-head">
                        <div class="card-title">Financial Performance (P&L Summary)</div>
                    </div>
                    <div class="pl-grid">
                        <div class="pl-item">
                        ...
                        <div class="pl-item">
                            ...
                            <div class="pl-val"
                                style="font-size: {{ $stats['operating_expenses'] == 0 ? '10px' : '15px' }}; color: {{ $stats['operating_expenses'] == 0 ? '#94a3b8' : '#1e293b' }}">
                        ...
                        <div class="pl-item">
                            ...
                            <div class="pl-val"
                                style="font-size: {{ $stats['cost_of_funds'] == 0 ? '10px' : '15px' }}; color: {{ $stats['cost_of_funds'] == 0 ? '#94a3b8' : '#1e293b' }}">
            ...
            <div class="body-right">
                <div class="card">
                    <div class="card-head">
                        <div class="card-title">Marketing & Referral Funnel</div>
                    </div>
                    <div class="funnel-list" style="padding:20px">
                        ...
                        @foreach($funnel as $f)
                            <div class="fn-step" style="margin-bottom:12px">
                                ...
                                <div class="kpi-progress">
                                    <div class="progress-bar" style="width:{{ $f['pct'] }}%; background: #3b82f6;"></div>
                                </div>
                            </div>
                        @endforeach
                    </div>
