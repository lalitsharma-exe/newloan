...
@section('bc')
<a href="{{ route('admin.products.index') }}">Products</a> / Edit: {{ $product->name }}
@endsection
@section('content')
<div style="max-width:760px">
<div class="card">
  <div class="card-hdr"><span class="card-title">Edit: {{ $product->name }}</span></div>
  <form method="POST" action="{{ route('admin.products.update', $product) }}">
    ...
    <div class="card-body">
      ...
      <div class="g2" style="gap:16px">
        ...
        <div class="fg" style="grid-column:span 2">
          ...
          <input type="text" name="name" class="fc" value="{{ old('name', $product->name) }}" required>
        </div>
        ...
        <div class="fg">
          ...
          <select name="interest_method" class="fc" required>
            ...
            <option value="reducing" {{ old('interest_method', $product->interest_method ?? 'flat') === 'reducing' ? 'selected' : '' }}>Reducing Balance</option>
          </select>
        </div>
        ...
        <div class="fg">
          ...
          <input type="number" name="initiation_fee_rate" class="fc" value="{{ old('initiation_fee_rate', $product->initiation_fee_rate ?? 40) }}" step="0.01" min="0" max="100" required>
          <span class="ft">Default: 40%</span>
        </div>

        <div class="fg">
          <label class="fl">Admin Fee (M per month, fixed) *</label>
          <input type="number" name="admin_fee_fixed" class="fc" value="{{ old('admin_fee_fixed', $product->admin_fee_fixed ?? 50) }}" step="0.01" min="0" required>
          <span class="ft">Default: M50/month</span>
        </div>

        <div class="fg">
          <label class="fl">Late Penalty (M per 10 days overdue)</label>
          <input type="number" name="late_payment_fee" class="fc" value="{{ old('late_payment_fee', $product->late_payment_fee ?? 20) }}" step="0.01" min="0">
        ...
        <div class="fg">
          ...
          <input type="number" name="min_term_months" class="fc" value="{{ old('min_term_months', $product->min_term_months) }}" min="1" max="6" required>
        </div>

        <div class="fg">
          <label class="fl">Max Term (months, max 6) *</label>
          <input type="number" name="max_term_months" class="fc" value="{{ old('max_term_months', $product->max_term_months) }}" min="1" max="6" required>
        </div>
        ...
        <div class="fg" style="grid-column:span 2">
          ...
          <textarea name="description" class="fc" rows="2">{{ old('description', $product->description) }}</textarea>
        </div>

        <div class="fg" style="grid-column:span 2">
          <label class="fl">Eligibility Criteria</label>
          <textarea name="eligibility_criteria" class="fc" rows="2">{{ old('eligibility_criteria', $product->eligibility_criteria) }}</textarea>
        </div>
    ...
    <div style="padding:16px 24px;border-top:1px solid var(--border);display:flex;gap:10px;justify-content:flex-end">
      ...
      <button type="submit" class="btn btn-p"><i class="bi bi-check-lg"></i> Update Product</button>
    </div>
  </form>
</div>
</div>
