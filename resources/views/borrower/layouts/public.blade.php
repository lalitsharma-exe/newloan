...
.header-logo img { height: 28px; width: auto; display: block; object-fit: contain; }
...
.header-apply {
  ...
  letter-spacing: .03em; text-decoration: none; transition: all .25s;
  display: flex; align-items: center; gap: 6px;
  white-space: nowrap; flex-shrink: 0;
}
.header-apply:hover { background: var(--accent2); box-shadow: 0 4px 14px rgba(140,198,63,.4); }
.header-apply i { font-size: 12px; }
...
#mobile-nav {
  ...
  background: rgba(5,11,26,.97); padding: 80px 28px 40px;
...
#mobile-nav a {
  ...
  padding: 13px 0; border-bottom: 1px solid rgba(255,255,255,.07); transition: color .2s;
}
...
.mobile-close {
  ...
  font-size: 20px; cursor: pointer; width: 38px; height: 38px;
  border-radius: 50%; display: flex; align-items: center; justify-content: center;
  transition: all .2s;
}
.mobile-close:hover { background: rgba(255,255,255,.15); color: #fff; }
.mobile-cta { display: flex; flex-direction: column; gap: 10px; margin-top: 28px; }
...
.footer-left a i, .footer-left p i {
  color: var(--light); font-size: 13px; width: 15px; text-align: center; flex-shrink: 0;
}
.footer-left p {
...
.footer-logo {
  ...
  margin: 0 auto 7px; filter: brightness(0) invert(1) opacity(.45);
}
.footer-copy { font-size: 12px; color: rgba(255,255,255,.25); margin-bottom: 7px; }
.footer-fb {
  display: inline-flex; align-items: center; gap: 6px;
  color: rgba(255,255,255,.4); text-decoration: none; font-size: 12px; transition: color .2s;
}
.footer-fb:hover { color: #4267B2; }

/* Payment logos */
.footer-right { display: flex; align-items: center; justify-content: flex-end; gap: 6px; flex-wrap: wrap; }
.pay-logo {
  background: rgba(255,255,255,.07); border: 1px solid rgba(255,255,255,.1);
  border-radius: 6px; padding: 5px 10px; height: 32px;
  display: flex; align-items: center; justify-content: center;
  transition: background .2s;
}
.pay-logo:hover { background: rgba(255,255,255,.12); }
.pay-logo img { height: 15px; width: auto; object-fit: contain; filter: brightness(0) invert(1) opacity(.55); }
.pay-logo-img-real img { filter: none; opacity: 1; }
.pay-logo span {
...
@media(max-width:1024px) {
  ...
  .footer-center { order: 3; grid-column: span 2; border-top: 1px solid rgba(255,255,255,.06); padding-top: 16px; margin-top: 4px; }
}
...
@media(max-width:768px) {
  ...
  .header-apply { padding: 7px 14px; font-size: 12px; }
  .header-apply .apply-text { display: none; }
  ...
  .footer-center { order: unset; border: none; padding: 0; margin: 0; }
  .footer-right { justify-content: center; }
}

@media(max-width:480px) {
  #header { padding: 6px 10px; }
  .header-pill { padding: 5px 5px 5px 12px; gap: 3px; }
  .header-logo img { height: 22px; }
  .header-apply { padding: 6px 12px; font-size: 11.5px; }
  .pay-logo { height: 28px; padding: 4px 8px; }
  .pay-logo img { height: 13px; }
  .pay-logo span { font-size: 9.5px; }
}
...
<nav id="mobile-nav" role="dialog" aria-label="Mobile navigation">
  ...
  <div class="mobile-cta">
    <a href="{{ route('borrower.login') }}" class="btn btn-outline-white" style="justify-content:center;border-radius:99px" onclick="toggleMobileNav()">
      <i class="bi bi-person-fill"></i> Sign In
    </a>
    <a href="{{ route('borrower.register') }}" class="btn btn-accent" style="justify-content:center;border-radius:99px" onclick="toggleMobileNav()">
...
<footer>
  <div class="footer-inner">
    ...
    <div class="footer-center">
      ...
      <a href="https://www.facebook.com/share/18TBN7SB8y/?mibextid=wwXIfr" target="_blank" rel="noopener" class="footer-fb">
    ...
    <div class="footer-right">
      ...
      <div class="pay-logo" title="Visa">
        <svg height="14" viewBox="0 0 48 16" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      <div class="pay-logo" title="Mastercard">
        <svg height="16" viewBox="0 0 38 24" fill="none" xmlns="http://www.w3.org/2000/svg">
          ...
          <path d="M19 5.5a10 10 0 0 1 0 13A10 10 0 0 1 19 5.5Z" fill="#FF5F00" opacity=".75"/>
        </svg>
      </div>
      <!-- CDAS real image -->
      <div class="pay-logo pay-logo-img-real" title="CDAS">
        <img src="https://ik.imagekit.io/ygydr1m84/WhatsApp%20Image%202026-03-22%20at%205.48.11%20PM%20(1).jpeg" alt="CDAS">
      </div>
      <!-- M-Pesa real image -->
...
<!-- ═══ WHATSAPP FLOAT ════════════════════════════════════ -->
<a href="https://wa.me/26663445244" target="_blank" rel="noopener" class="whatsapp-float" title="Chat on WhatsApp" aria-label="Chat with us on WhatsApp">
