...
<div>
  <div class="tabs">
    ...
    <button class="tab" data-tg="app" data-t="loan"     onclick="switchTab('app','loan')"><i class="bi bi-bank"></i> Loan</button>
    ...
    <button class="tab" data-tg="app" data-t="credit"   onclick="switchTab('app','credit')"><i class="bi bi-graph-up"></i> Credit / Risk</button>
  </div>
  ...
  <div class="tpanel active" data-pg="app" data-p="personal">
    ...
    <div class="card" style="margin-bottom:16px">
      <div class="card-hdr"><span class="card-title"><i class="bi bi-people-fill" style="color:var(--p)"></i> Next of Kin</span></div>
      <div class="card-body">
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:14px">
          @foreach($application->nextOfKin as $k)
          <div style="background:#f8fafc;border-radius:10px;padding:14px;border:1px solid var(--border)">
            <div style="font-weight:700;font-size:13.5px;margin-bottom:4px">{{ $k->first_name }} {{ $k->last_name }}</div>
            <div style="font-size:12px;color:var(--p);font-weight:600;margin-bottom:3px">{{ ucfirst($k->relationship) }}</div>
            <div style="font-size:12.5px;color:var(--muted)"><i class="bi bi-phone"></i> {{ $k->contact_number }}</div>
          </div>
          @endforeach
        </div>
      </div>
    ...
    <div class="card" style="margin-bottom:16px">
      <div class="card-hdr"><span class="card-title"><i class="bi bi-geo-alt-fill" style="color:var(--p)"></i> Address</span></div>
      <div class="card-body" style="font-size:13.5px;line-height:1.6;white-space:pre-line">{{ $application->user->address }}</div>
    </div>
    @endif
  </div>
  ...
  {{-- CREDIT / RISK --}}
  <div class="tpanel" data-pg="app" data-p="credit">

    {{-- Read-only notice --}}
    <div class="alert a-i" style="margin-bottom:16px">
      <i class="bi bi-shield-lock-fill"></i>
      <div>Credit bureau data and risk scores are <strong>visible to officers</strong> for assessment purposes. Only admins can modify scores or attach reports.</div>
    </div>
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;margin-bottom:16px">
      ...
      {{-- Risk Score --}}
      <div class="card">
        <div class="card-hdr"><span class="card-title"><i class="bi bi-graph-up" style="color:var(--p)"></i> Risk Score</span></div>
        <div class="card-body" style="text-align:center">
          ...
          @endphp
          <div style="font-size:52px;font-weight:800;color:{{ $rc }};line-height:1;margin-bottom:8px">{{ $rs }}</div>
          <div style="font-size:14px;font-weight:700;color:{{ $rc }};margin-bottom:14px">{{ $rl }}</div>
          <div style="height:10px;background:var(--bg);border-radius:99px;overflow:hidden;margin-bottom:6px">
            <div style="height:100%;width:{{ $rp }}%;background:{{ $rc }};border-radius:99px;transition:width .6s ease"></div>
          </div>
          <div style="display:flex;justify-content:space-between;font-size:10px;color:var(--muted)">
            <span>0 – High Risk</span><span>1000 – Low Risk</span>
          </div>
          @else
          <div style="padding:30px;color:var(--muted)">
            <i class="bi bi-graph-up" style="font-size:40px;opacity:.2;display:block;margin-bottom:10px"></i>
            <div style="font-size:13px">No risk score assigned yet</div>
            <div style="font-size:12px;margin-top:4px">Admin will set this during review</div>
          </div>
          @endif
        </div>
      </div>
      ...
      {{-- Credit Report --}}
      <div class="card">
        <div class="card-hdr"><span class="card-title"><i class="bi bi-file-earmark-bar-graph" style="color:var(--p)"></i> Credit Report</span></div>
        <div class="card-body">
          @if($application->creditReport)
          @php $cr = $application->creditReport; @endphp
          <div style="display:flex;flex-direction:column;gap:10px">
            @foreach([
              'Credit Score'    => $cr->credit_score ?? '—',
              'Risk Category'   => ucfirst($cr->risk_category ?? '—'),
              'Outstanding Debt'=> $cr->outstanding_debts ? 'M'.number_format($cr->outstanding_debts,2) : '—',
              'Past Defaults'   => $cr->past_defaults ?? '—',
              'Pulled On'       => $cr->created_at?->format('d M Y') ?? '—',
            ] as $label => $val)
            <div style="display:flex;justify-content:space-between;padding:7px 0;font-size:12.5px;border-bottom:1px solid var(--border)">
              ...
              <strong style="{{ $label==='Risk Category'?'color:'.($cr->risk_category==='low'?'#10b981':($cr->risk_category==='high'?'#ef4444':'#f59e0b')):'' }}">{{ $val }}</strong>
            </div>
            @endforeach
            @if($cr->notes)
            <div style="background:#f8fafc;border-radius:9px;padding:12px;font-size:12.5px;color:#374151;line-height:1.5;margin-top:4px">
              <div style="font-size:11px;font-weight:700;text-transform:uppercase;color:var(--muted);letter-spacing:.05em;margin-bottom:5px">Assessor Notes</div>
              {{ $cr->notes }}
            </div>
            @endif
          </div>
          ...
          <div style="text-align:center;padding:30px;color:var(--muted)">
            <i class="bi bi-file-earmark-bar-graph" style="font-size:40px;opacity:.2;display:block;margin-bottom:10px"></i>
            <div style="font-size:13px">No credit report on file</div>
            <div style="font-size:12px;margin-top:4px">Admin pulls this via Experian during review</div>
          </div>
          @endif
        </div>
      </div>
    ...
    {{-- Employment & income context for risk --}}
    @if($application->employment && $application->affordability)
    <div class="card">
      <div class="card-hdr"><span class="card-title"><i class="bi bi-briefcase" style="color:var(--p)"></i> Income & Employment Summary</span></div>
      <div class="card-body">
        <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:14px">
          @php $af = $application->affordability; @endphp
          @foreach([
            ['Employer',        $application->employment->employer_name,             '#4f46e5'],
            ['Gross Earnings',  'M'.number_format($af->monthly_earnings??0,2),       '#0891b2'],
            ['Net Pay',         'M'.number_format($af->net_salary??0,2),             'var(--p)'],
            ['Disposable',      'M'.number_format($af->disposable_income??0,2),      $af->disposable_income>=0?'#10b981':'#ef4444'],
          ] as [$lbl,$val,$color])
          <div style="background:#f8fafc;border-radius:11px;padding:14px;text-align:center;border:1px solid var(--border)">
            <div style="font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:.06em;color:var(--muted);margin-bottom:6px">{{ $lbl }}</div>
            <div style="font-size:16px;font-weight:800;color:{{ $color }};overflow:hidden;text-overflow:ellipsis;white-space:nowrap">{{ $val }}</div>
          </div>
          @endforeach
        </div>
      </div>
