...
<title>Sign In — {{ config('app.name') }} Officer</title>
...
.brand-logo { position: relative; z-index: 1; display: flex; align-items: center; gap: 12px; }
.brand-logo-icon { width: 44px; height: 44px; background: linear-gradient(135deg,#1a6b3c,#22894e); border-radius: 11px; display: flex; align-items: center; justify-content: center; box-shadow: 0 3px 12px rgba(0,0,0,.3); flex-shrink: 0; }
.brand-logo-text { font-family: 'Playfair Display', serif; font-size: 18px; font-weight: 700; color: #fff; }
.brand-logo-sub { font-size: 10px; color: rgba(255,255,255,.45); letter-spacing:.06em; text-transform:uppercase; }
...
.brand-pill-icon {
  ...
  justify-content: center; color: #e0a843; font-size: 16px;
}
...
.auth-success {
  ...
  color: #15803d; padding: 11px 14px; border-radius: 8px;
  font-size: 13px; margin-bottom: 18px;
  display: flex; align-items: center; gap: 8px;
}
...
.portal-link {
  ...
  border: 1.5px solid #d4e0d4; color: #5a6b85; background: #fff;
}
.portal-link:hover { border-color: #1a6b3c; color: #1a6b3c; background: rgba(26,107,60,.04); }
.portal-link.active { border-color: #1a6b3c; color: #1a6b3c; background: rgba(26,107,60,.06); }
...
@media(max-width: 768px) {
  ...
  .portal-link:hover { border-color: rgba(255,255,255,.4); color: #fff; }
}
</style>
</head>
<body>
  ...
  <div class="auth-brand">
    <div class="brand-logo">
      ...
      <div>
        ...
        <div class="brand-logo-sub">Officer Portal</div>
      </div>
    </div>

    <div class="brand-body">
      <h2>Loan <em>Officer</em><br>Work Portal</h2>
      <p>Manage assigned loans, review applications, track collections and communicate with borrowers efficiently.</p>
      <div class="brand-pills">
        <div class="brand-pill">
          <div class="brand-pill-icon"><i class="bi bi-clipboard-check-fill"></i></div>
          <div class="brand-pill-text"><strong>Application Review</strong><span>Process and assess loan applications</span></div>
        </div>
        <div class="brand-pill">
          <div class="brand-pill-icon"><i class="bi bi-people-fill"></i></div>
          <div class="brand-pill-text"><strong>Borrower Management</strong><span>Track and support your assigned clients</span></div>
        </div>
        <div class="brand-pill">
          <div class="brand-pill-icon"><i class="bi bi-graph-up-arrow"></i></div>
          <div class="brand-pill-text"><strong>Performance Tracking</strong><span>Monitor your collection targets</span></div>
        </div>
      </div>
    </div>
  ...
  <div class="auth-form-wrap">
    <div class="auth-form">
      ...
      <div class="auth-head">
        ...
        <p>Sign in to the loan officer portal.</p>
      </div>
      ...
      <form method="POST" action="{{ route('officer.login.post') }}">
        ...
        <div class="fg">
          ...
          <div class="field-wrap">
            ...
            <button type="button" onclick="togglePw()" style="position:absolute;right:12px;top:50%;transform:translateY(-50%);background:none;border:none;cursor:pointer;color:#8aaa8a;font-size:15px" id="pwToggle">
              <i class="bi bi-eye"></i>
            </button>
          </div>
        </div>

        <a href="{{ route('officer.password.request') }}" class="forgot-link">Forgot password?</a>
        ...
        <button type="submit" class="btn-submit">
          <i class="bi bi-person-badge-fill"></i> Sign In as Officer
        </button>
      </form>
      ...
      <div class="portal-switch">
        ...
        <a href="{{ route('officer.login') }}" class="portal-link active">
          <i class="bi bi-person-badge-fill"></i> Officer
        </a>
        <a href="{{ route('admin.login') }}" class="portal-link">
